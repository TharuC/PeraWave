...
const loginUser = async (req, res) => {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ userId: user.id, email: user.email, role: 'USER' }, process.env.JWT_SECRET || 'fallback_secret', { expiresIn: '24h' });
...
const modRegister = async (req, res) => {
    try {
        ...
        const newMod = await db_1.default.moderator.create({
            data: {
                email,
                password: hashedPassword,
                fullName: name,
...
exports.modRegister = modRegister;
const modLogin = async (req, res) => {
    try {
        ...
        const moderator = await db_1.default.moderator.findUnique({ where: { email } });
        if (!moderator) {
            return res.status(401).json({ error: 'Invalid email or password' });
        }
        const isPasswordValid = await bcrypt_1.default.compare(password, moderator.password);
        if (!isPasswordValid) {
            return res.status(401).json({ error: 'Invalid email or password' });
        }
        const token = jsonwebtoken_1.default.sign({ userId: moderator.id, email: moderator.email, role: 'MODERATOR' }, process.env.JWT_SECRET || 'fallback_secret', { expiresIn: '24h' });
        return res.status(200).json({
...
const getCurrentUser = async (req, res) => {
    try {
        ...
        if (role === 'MODERATOR') {
            const mod = await db_1.default.moderator.findUnique({ where: { id: userId } });
            if (!mod)
