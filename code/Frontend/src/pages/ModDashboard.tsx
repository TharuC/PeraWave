...
const ModDashboard: React.FC = () => {
    ...
    const fetchAll = async () => {
        ...
        try {
            ...
            // 2. Platform stats
            const statsRes = await fetch('http://localhost:8080/api/mod/stats', { headers });
            if (statsRes.ok) setStats(await statsRes.json());

            // 3. Users list
            const usersRes = await fetch('http://localhost:8080/api/mod/users', { headers });
            if (usersRes.ok) setUsers(await usersRes.json());

            // 4. Forum posts
            setPostsLoading(true);
            const postsRes = await fetch('http://localhost:8080/api/forum/posts', { headers });
            if (postsRes.ok) setForumPosts(await postsRes.json());
            setPostsLoading(false);

            // 5. Audit logs
            setAuditLoading(true);
            const auditRes = await fetch('http://localhost:8080/api/mod/audit-logs', { headers });
